...
class Attendance:
    ...
    def __init__(self,root):
        ...
        self.root.title("Attendence Pannel")
        ...
        # backgorund image 
        bg1=Image.open(r"College img\bg2.jpg")
        ...
        #title section
        title_lb1 = Label(bg_img,text="Welcome to Attendence Pannel",font=("verdana",30,"bold"),bg="white",fg="navyblue")
        ...
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    mycursor.execute("update studentatt set id=%s,roll=%s,name=%s,time=%s,date=%s,attend=%s where id=%s",( 
                    self.var_id.get(),
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_time.get(),
        ...
        if self.var_id.get()=="":
            ...
            try:
                ...
                if delete>0:
                    ...
                    mycursor = conn.cursor() 
                    sql="delete from studentatt where std_id=%s"
                    val=(self.var_id.get(),)
    ...
    def fetch_data(self):
        ...
        mycursor.execute("select * from studentatt")
    ...
    def reset_data(self):
        ...
        self.attendanceReport_left.delete(0,END)
    ...
    def action(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                conn = mysql.connector.connect(username='root', password='Shivam123',host='localhost',database='face_recognition_system')
                mycursor = conn.cursor()
                mycursor.execute("insert into studentatt values(%s,%s,%s,%s,%s,%s)",(
                self.var_id.get(),
                self.var_roll.get(),
                self.var_name.get(),
                self.var_time.get(),
